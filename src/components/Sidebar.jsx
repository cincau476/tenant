...
const Sidebar = ({ children, expanded, setExpanded }) => {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-screen z-50 transition-all ${expanded ? "w-72" : "w-20"}`}>
      <nav className="h-full flex flex-col bg-white border-r shadow-sm">
        ...
        <SidebarContext.Provider value={{ expanded }}>
          <ul className="flex-1 px-3 overflow-y-auto min-h-0">{children}</ul>
        </SidebarContext.Provider>
        
        <div className="border-t flex p-3">
            <img 
                src="https://ui-avatars.com/api/?background=c7d2fe&color=3730a3&bold=true&name=Admin+User"
                alt=""
            ...
            <div className={`flex justify-between items-center overflow-hidden transition-all ${expanded ? "w-52 ml-3" : "w-0"}`}>
                <div className="leading-4">
                    ...
                    <span className="text-xs text-gray-600">admin@orderin.app</span>
                </div>
